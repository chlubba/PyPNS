...
if calculationFlag:
    ...
    for i in [0,1]:
        ...
        for widthInd, cuffWidth in enumerate(cuffWidths):
            ...
            zValues = np.linspace(-0.02, 0.02, 10000)
            ...
            vValues = smooth(vValues, smoothLengh)
        ...
        cNorm = colors.Normalize(vmin=0, vmax=len(bundle.recordingMechanisms) - 1)
        ...
        for recMechIndex in range(len(bundle.recordingMechanisms)):
            ...
            axarr[i].plot(t, SFAPs, label=str(cuffWidths[recMechIndex]*2*1000)+' mm', color=colorVal)
        ...
        titles = ['unmyelinated, profile smoothed over'+ str(smoothLengh*4) + ' um', 'myelinated, profile smoothed over'+ str(smoothLengh*4) + ' um']
        axarr[i].set_title(titles[i])

        if i == 0:
            axarr[i].set_xlim([24,32])
        elif i == 1:
            axarr[i].set_xlim([0,7])
