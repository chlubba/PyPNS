...
def retrieve_mono_sum_CAP(filename):
    with open(filename, 'r') as f:
        ...
        numCols = len(secondRow)
    sum_CAP = np.loadtxt(filename, unpack=True, usecols=range(1,numCols))
    return sum_CAP

ax = []
# verticalPlotCount=voltageFlag+CAPFlag
...
if CAPFlag:
    ...
    for k in range(3):
        ax.append(fig.add_subplot(gs[3+subfigureOffset+k]))
        V= None
        t = None
        filename = tp[k][1]
        print filename

        bundleParams=retrieve_parameters(filename)
        numberOfElectrodes=bundleParams['number_elecs']

        cNorm = colors.Normalize(vmin=0, vmax=numberOfElectrodes-1)#len(diameters_m)-1)#
        scalarMap = cm.ScalarMappable(norm = cNorm, cmap= jet)

        t = retrieve_time(filename)
        V = retrieve_mono_sum_CAP(filename)

        voltageArray=np.array(V*1000)
        for j in range(numberOfElectrodes):
            ...
            ax[k+subfigureOffset].plot(np.array(t), voltageArray[j,:], color=colorVal,  linestyle='-', linewidth = 1)
            ax[k+subfigureOffset].set_xlim([5,30])
            ax[k+subfigureOffset].set_ylim([-2.00,1.00])
        ax[k+subfigureOffset].spines['right'].set_visible(False)
        ax[k+subfigureOffset].spines['top'].set_visible(False)
        ax[k+subfigureOffset].xaxis.set_ticks_position('none')
        ax[k+subfigureOffset].yaxis.set_ticks_position('none')
        if k==0:
            ax[k+subfigureOffset].yaxis.set_ticks_position('left')
            ax[k+subfigureOffset].set_ylabel('Extracellular potential ['+r'$\mu$'+'V]', fontsize=text_size, fontweight = 'bold', labelpad=0)
        else:
            ax[k+subfigureOffset].spines['left'].set_visible(False)
            for ylabel_i in ax[k+subfigureOffset].get_yticklabels():
                ylabel_i.set_visible(False)
                ylabel_i.set_fontsize(0.0)


        ax[k+subfigureOffset].xaxis.set_ticks_position('bottom')
        ax[k+subfigureOffset].set_xlabel('Time [ms]', fontsize=text_size, fontweight = 'bold', labelpad=0)
        if k == 0:
            tt = "Extracellular \n Monophasic 0.01 ms, 1 mA"
        elif k==1:
            tt = "Intracellular \n Monophasic 0.1 ms, 2 mA"
        ...
        ax[k+subfigureOffset].set_title(tt, y=0.82,fontsize =text_size+2, fontweight = 'bold',verticalalignment="bottom")
