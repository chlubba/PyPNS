...
for j in range(6):
    ...
    ax[j].set_title('Distance from stimulus '+str(tp[j+2][0][0]/1000.0)+ ' mm',fontsize =text_size+2, fontweight = 'bold',verticalalignment="bottom")
...
CAP5 = retrieve_mono_sum_CAP(tp[3][1])
for j in range(4):
    
    CAP = retrieve_mono_sum_CAP(tp[j+4][1])
    CAP_bipo = CAP-CAP5
    ax.append(fig.add_subplot(gs[j]))
    ...
    if j < 2:
        ...
        ax[j].xaxis.set_ticks_position('bottom')
        ax[j].set_xlabel('Time [ms]', fontsize=text_size, fontweight = 'bold', labelpad=0)
    ax[j].set_title('Distance between poles '+str(tp[j+4][0][0]/1000.0-tp[3][0][0]/1000.0)+ ' mm',fontsize =text_size+2, fontweight = 'bold',verticalalignment="bottom")
plt.suptitle('Distance first pole from stimulus '+str(tp[3][0][0]/1000.0)+ ' mm',fontsize =text_size+3, fontweight = 'bold',verticalalignment="bottom", y = 0.94, style="italic")
fig.subplots_adjust(hspace=0.1,wspace=0.04)
...
ax =[]
for j in range(4):
    CAP0 = retrieve_mono_sum_CAP(tp[j+3-1][1])
    CAP = retrieve_mono_sum_CAP(tp[j+3+1][1])
    CAP_bipo = CAP-CAP0
    ...
    if j < 2:
        ...
        ax[j].xaxis.set_ticks_position('bottom')
        ax[j].set_xlabel('Time [ms]', fontsize=text_size, fontweight = 'bold', labelpad=0)
    ax[j].set_title('Distance first pole from stimulus '+str(tp[j+3-1][0][0]/1000.0)+ ' mm',fontsize =text_size+2, fontweight = 'bold',verticalalignment="bottom")
plt.suptitle('Distance between poles '+str(tp[j+3+1][0][0]/1000.0-tp[j+3-1][0][0]/1000.0)+ ' mm',fontsize =text_size+3, fontweight = 'bold',verticalalignment="bottom", y = 0.94, style="italic")
...
for j in range(4):
    ...
    CAP = retrieve_mono_sum_CAP(tp[j+2+2][1])
    CAP_bipo = CAP-CAP0
...
plt.suptitle('Distance between poles '+str(tp[j+2+2][0][0]/1000.0-tp[j+2-1][0][0]/1000.0)+ ' mm',fontsize =text_size+3, fontweight = 'bold',verticalalignment="bottom", y = 0.94, style="italic")
fig.subplots_adjust(hspace=0.1,wspace=0.04)
