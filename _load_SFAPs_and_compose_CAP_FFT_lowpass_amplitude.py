...
for fieldTypeInd in [0]: # fieldTypes:
    ...
    denum = np.concatenate(([1], np.array([1.14384031490999e-06, -6960.76247988057, -0.00799087266204879, 173188542.754159, 17.6610164642334,
             -289826464890.561])))

    from scipy import signal

    plt.figure()
    w, h = signal.freqz(denum, num)
    plt.plot(w, 20 * np.log10(abs(h)))
    # plt.xscale('log')
    plt.show()

    filteredCAP = lfilter(denum, num, CAP)
    f, Pxx_den = signal.welch(filteredCAP - np.mean(filteredCAP), 1000. / (tCAP[1] - tCAP[0]), nperseg=1024)
    plt.semilogy(f, Pxx_den / np.max(Pxx_den), label='Matlab')
    ...
    for order in [1,2,3,4]:
        ...
        f, Pxx_den = signal.welch(filteredCAP - np.mean(filteredCAP), 1000. / (tCAP[1] - tCAP[0]), nperseg=1024)
        plt.semilogy(f, Pxx_den / np.max(Pxx_den), label='order='+str(order))
        # plt.ylim([0.5e-3, 1])
        plt.xlabel('frequency [Hz]')
        plt.ylabel('PSD [V**2/Hz]')
