...
saveDict = pickle.load(open(os.path.join('SFAPs', 'SFAPsPowleyMyelAsRecordingsIdealizedCuff.dict'), "rb" )) # thinnerMyelDiam # SFAPsPowleyMyelAsRecordingsNewCurr
...
tHomo = saveDict['t']
ts = [tHomo, np.arange(0,10,0.0025), np.arange(0,10,0.0025)]
...
f_rec, Pxx_den_rec = signal.welch(data[:,1] - np.mean(data[:,1]), 1./(time[1]-time[0]), nperseg=256)
