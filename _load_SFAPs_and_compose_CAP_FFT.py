...
saveDict = pickle.load(open(os.path.join('/media/carl/4ECC-1C44/PyPN/SFAPs', 'SFAPsPowleyMyelAsRecordings.dict'), "rb" )) # thinnerMyelDiam
...
for fieldTypeInd in fieldTypes:
    for typeInd in [0, 1]:
        ...
        sp = np.fft.fft(CAP)
        freq = np.fft.fftfreq(CAP.shape[-1], d=dt)
        ...
        axarr[typeInd].semilogy(freq, absSp / max(absSp), label='simulated ' + typeStrings[typeInd])

        if typeInd == 0:
            recSignal = vDenCut[np.logical_and(tCut>12, tCut<26)]

        else:
            recSignal = vDenCut[np.logical_and(tCut>30, tCut<140)]

        dtRec = tCut[1] - tCut[0]

        sp = np.fft.fft(recSignal)
        freq = np.fft.fftfreq(recSignal.shape[-1], d=dtRec)
        ...
        axarr[typeInd].semilogy(freq, absSp / max(absSp), label='recorded ' + typeStrings[typeInd])

        axarr[typeInd].legend()
        axarr[typeInd].grid()
