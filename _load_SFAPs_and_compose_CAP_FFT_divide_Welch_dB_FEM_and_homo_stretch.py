...
f_rec, Pxx_den_rec = signal.welch(vDenCutPlot - np.mean(vDenCutPlot), 1./(time[1]-time[0]), nperseg=200)
...
for fieldTypeInd in [0,1]: # fieldTypes:
    ...
    # plt.plot(f_finer, 20 * np.log10(
    #     np.sqrt(Pxx_div_finer / np.max(Pxx_div_finer))), label='rec/sim')
    plt.xlabel('frequency [Hz]')
