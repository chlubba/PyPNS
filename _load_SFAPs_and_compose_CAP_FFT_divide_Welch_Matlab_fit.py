...
for fieldTypeInd in [0]: # fieldTypes:
    ...
    interpolator_Pxx_div = interp1d(f_common, Pxx_divided)#, kind='quadratic')
    Pxx_div_finer = interpolator_Pxx_div(f_finer)

    # # np.savetxt('/home/carl/Desktop/Matlab/spectral_data.dat', np.column_stack([f_common, Pxx_den_rec_interp, Pxx_den_cut]))
    # np.savetxt('/home/carl/Desktop/Matlab/spectral_data.dat', np.column_stack([f_finer, np.sqrt(Pxx_div_finer)]))

    # get the poles and zeros from matlab
    num = -18160993481857.9
    denum = [1, 1.14384031490999e-06, -6960.76247988057, -0.00799087266204879, 173188542.754159, 17.6610164642334, -289826464890.561]

    # plt.semilogy(f_common, Pxx_den_rec_interp/np.max(Pxx_den_rec_interp)/(Pxx_den_cut / np.max(Pxx_den_cut)), label='rec/sim')
    plt.plot(f_common, 20 * np.log10(np.sqrt(Pxx_den_rec_interp / np.max(Pxx_den_rec_interp) / (Pxx_den_cut / np.max(Pxx_den_cut)))),
                 label='rec/sim')
    plt.plot(f_finer, 20 * np.log10(np.sqrt(Pxx_div_finer)), label='rec/sim')
    plt.xlabel('frequency [Hz]')
    plt.ylabel('PSD [V**2/Hz]')
