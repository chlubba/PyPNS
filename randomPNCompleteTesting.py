...
def create_random_axon(bundleCoords, bundleRadius, segmentLengthAxon, diameter = 1, maximumAngle = 0.314):
    ...
    rhoMax = tan(maximumAngle)*segmentLengthAxon #0#
    rhoArray = np.zeros(5)
    ...
    while currentBundleSegment <= bundleLength-1:
        ...
        correctionVector = radiusVectorNorm + 4*bundleDirectionNorm
        ...
        combinedDirection = combinedDirection/np.linalg.norm(combinedDirection)
        ...
        # rho = np.random.uniform(1)*rhoMax
        randomDirection = combinedDirection + 0.05*randomOrthogonalVectorNorm*rho
...
bundleSegmentLength = segmentLengthAxon*3

numBundleGuideSteps = int(np.floor(bundleLength/bundleSegmentLength))
bundleGuideStepSize = bundleSegmentLength

halfIndex = numBundleGuideSteps/2
...
bundleCoords[:,0] = range(0, numBundleGuideSteps*bundleGuideStepSize, bundleGuideStepSize)
bundleCoords[:,1] = np.zeros(numBundleGuideSteps)
bundleCoords[:,2] = np.zeros(numBundleGuideSteps)
# bundleCoords[:,1] = np.concatenate((np.zeros(turningPointIndex1),np.multiply(range(turningPointIndex2), bundleGuideStepSize)))
# bundleCoords[:,2] = np.concatenate((np.zeros(turningPointIndex1),np.multiply(range(turningPointIndex2), bundleGuideStepSize)))
...
max_range = np.array([bundleLength/2, bundleLength/2, bundleLength/2]).max()
Xb = 0.5*max_range*np.mgrid[-1:2:2,-1:2:2,-1:2:2][0].flatten()# + 0.5*(bundleLength)
Yb = 0.5*max_range*np.mgrid[-1:2:2,-1:2:2,-1:2:2][1].flatten()# + 0.5*(bundleLength)
Zb = 0.5*max_range*np.mgrid[-1:2:2,-1:2:2,-1:2:2][2].flatten()# + 0.5*(bundleLength)
