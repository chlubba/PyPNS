...
lengthOfBundle = 30000 # 20000 # 400000
numberOfAxons = 1
...
if calculationFlag:
    ...
    for elecInd, elecRadius in enumerate([235]): # [400, 500, 600, 700, 1000] [50, 100, 150, 200, 250, 300]

        print 'radius ' + str(elecRadius)

        # ----------------------------- recording params -------------------------------

        recordingParametersNew = {'bundleGuide': bundleGuide,
                                  'radius': elecRadius, # 250,
                                  'positionAlongBundle': 15000,
                                  'numberOfPoles': 1,
                                  'poleDistance': 1000,
                                  }

        # plotRow = int(elecInd / 3)
        # plotColumn = np.mod(elecInd, 3)
        ...
        for RDCInd, RDC in enumerate(RDCs):
            ...
            for runInd in range(numRuns):
                print 'RDC number ' + str(RDCInd)
                print 'run ' + str(runInd)

                vAPs = []

                LFPMechs = []
                LFPMechs.append(PyPN.Extracellular.homogeneous(sigma=1))
                LFPMechs.append(PyPN.Extracellular.precomputedFEM(bundleGuide))
                LFPMechs.append(PyPN.Extracellular.interpolator(bundleGuide))

                electrodePos = PyPN.createGeometry.circular_electrode(**recordingParametersNew)

                modularRecMechs = []
                for i in range(len(LFPMechs)):
                    modularRecMechs.append(PyPN.RecordingMechanism(electrodePos, LFPMechs[i]))
                ...
                myelinatedDiam = 3.  # {'distName' : 'normal', 'params' : (1.0, 0.7)} # (2.0, 0.7)
                unmyelinatedDiam = 3.  # {'distName' : 'normal', 'params' : (0.7, 0.3)}
                ...
                for i in range(len(modularRecMechs)):
                    bundle.add_recording_mechanism(modularRecMechs[i])
                ...
                for i in range(len(modularRecMechs)):
                    ...
                    # plt.plot(t, SFAPs)
                    SFAPArray.append(SFAPs)
                # plt.show()

                RDCSave.append(RDC)
                elecDistSave.append(elecRadius)
                signalsSave.append(SFAPArray)
        ...
        #         SFAPReg = f(tReg)
        #
        #         SFAPActive = np.where(np.abs(SFAPReg) > 0.00000001)[0]
...
saveDict = {'signals': signalsSave,
            'RDCs': RDCSave,
            'radius': elecDistSave}



pickle.dump(saveDict, open(os.path.join('tortuous', 'tortuousDTconstUnmyel.dict'), "wb"))
