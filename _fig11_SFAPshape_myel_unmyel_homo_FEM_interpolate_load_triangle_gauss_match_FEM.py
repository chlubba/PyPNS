...
if calculationFlag:
    ...
    for i in [0,1]:
        ...
        profileParams = [smoothingLength, sigmas, FEMFileNames]
        ...
        for profileInd in [0,1,2]: # [0, 1, 2]
            ...
            for profileParamInd, profileParam in enumerate(profileParams[profileInd]):
                ...
                if profileInd == 0:
                    ...
                    profileTitle = 'gaussian, sigma in um'
                    profileLegends = (np.array(profileParams[profileInd]) * 500000).astype(str)

                    mu = 0.
                    vValues = 1/(profileParam * np.sqrt(2 * np.pi)) * np.exp( - (zValues - mu)**2 / (2 * (profileParam)**2) )
                    vValues = vValues - np.min(vValues) # make sure it goes down to zero
                    vValues = vValues/np.max(vValues) * 8.83e-5 # scale to fit FEM
