...
timePlotMin = 14400 # 10
timePlotMax = 14550 # 35
...
plt.semilogy(f, Pxx_den_rec/np.max(Pxx_den_rec), label='recording', color='r')
# plt.ylim([0.5e-3, 1])
...
for fieldTypeInd in [0]: # fieldTypes:
    ...
    plt.semilogy(f, Pxx_den/np.max(Pxx_den), label='simulation raw', color='b')
    # plt.ylim([0.5e-3, 1])
    plt.xlabel('frequency [Hz]')
    plt.ylabel('PSD [V**2/Hz]')
    # plt.show()
    ...
    # filter
    linestyles = ['-', '--', '-.', ':']
    for orderInex , order in enumerate([1,2,3,4]):

        filteredCAP = butter_lowpass_filter(CAP, 100, 1000./dt, order=order)

        f, Pxx_den = signal.welch(filteredCAP - np.mean(filteredCAP), 1000. / (tCAP[1] - tCAP[0]), nperseg=1024)
        plt.semilogy(f, Pxx_den / np.max(Pxx_den), label='order='+str(order), color='g', linestyle=linestyles[orderInex])
        # plt.ylim([0.5e-3, 1])
        plt.xlabel('frequency [Hz]')
        plt.ylabel('PSD [V**2/Hz]')
...
plt.title('Spectra of recording, simulation and filtered simulation')
plt.grid()
plt.legend()
