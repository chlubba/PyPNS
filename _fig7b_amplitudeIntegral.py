...
diametersUnmyel = np.arange(0.2, 2, 0.3) # [0.5, 1] #
diametersMyel = np.arange(0.2, 4, 0.3) # [2.3, 2.6, 2.9] #
...
if calculationFlag:
    ...
    for i in [0,1]:
        ...
        for recMechIndex in [0,1]:
            print recMechLegends[recMechIndex]
            ...
            for diameterInd, diameter in enumerate(diameters):
                print 'diameter ' + str(diameterInd)
                ...
                modularRecMech = [[] for jj in range(2)]
                if i == 1:
                    ...
                    for ii in [0,1]:
                        ...
                        recordingParametersNew = {'bundleGuide': bundle.bundleCoords,
                                                  ...
                                                  'positionAlongBundle': np.floor(10000. / bundle.axons[0].lengthOneCycle) *
                                                                         bundle.axons[0].lengthOneCycle + bundle.axons[0].lengthOneCycle/2*ii,
                                                  'numberOfPoles': 1,
                                                  'poleDistance': 1000,
                                                  }
                ...
                SFAPNoArtScaled = np.squeeze(SFAPs[selectionArray])

                # plt.plot(SFAPNoArtScaled)
                # plt.title(str(diameter))
                # plt.show()

                # y_int = integrate.cumtrapz(tNoArt, np.abs(SFAPNoArtScaled), initial=0)
                # integralSFAP = y_int[-1]

                integralSFAP = np.sum(np.abs(SFAPNoArtScaled))*0.0025


                vAPs.append(integralSFAP)
                print vAPs
            ...
            if i == 0:
                ...
                # plt.semilogy(diameters, vAPs2, marker=recMechMarkers[recMechIndex], color='r', label=legends[i]+ ' between nodes' + ' ' + recMechLegends[recMechIndex])
    ...
    plt.ylabel('$\int \, |V_{ext}| \, dt$ [mV ms]')
    plt.legend(loc='best')
    plt.grid()
    plt.show()
else:
