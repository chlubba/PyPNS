...
window = signal.blackmanharris(len(vDenCut))
vDenCut = np.multiply(window, vDenCut)

dtRec = tCut[1] - tCut[0]
sp = np.fft.fft(vDenCut)
freqRec = np.fft.fftfreq(vDenCut.shape[-1], d=dtRec/1000)

sortedInds = sorted(range(len(freqRec)), key=lambda k: freqRec[k])
absSpRec = np.abs(sp)
absSpRec = absSpRec[sortedInds]
absSpRec = smooth(absSpRec,50)
# absSpRec = absSpRec/np.max(absSpRec)
freqRec = freqRec[sortedInds]

plt.semilogy(freqRec, absSpRec, label='recording')
# plt.show()
...
if calculationFlag:
    ...
    for typeInd in [0, 1]:
        ...
        for diameterInd, diameter in enumerate(diameters):
            ...
            window = signal.blackmanharris(len(iSignal))
            iSignal = np.multiply(window, iSignal)

            sp = np.abs(np.fft.fft(iSignal))
            # sp = sp / np.max(sp)
            ...
            # for ii in range(3):
            #     plt.plot(t, imem[nodeIndex + ii, :], label=labelStringsSections[ii])
            # plt.plot(t, np.sum(imem[nodeIndex:nodeIndex+3, :], axis=0), label='Sum of all', linewidth=1.5, color='k')
            #
            # plt.xlim((0.15, 0.9))
