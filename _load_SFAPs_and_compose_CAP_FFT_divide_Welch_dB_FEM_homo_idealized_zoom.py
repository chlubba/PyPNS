...
saveDict = pickle.load(open(os.path.join('SFAPs', 'SFAPsPowleyMyelAsRecordingsIdealizedCuff2.dict'), "rb" )) # thinnerMyelDiam # SFAPsPowleyMyelAsRecordingsNewCurr
...
electrodeDistance = 70.*1.2 # mm
jitterAmp = 5 #ms
jitterDist = 0.1*electrodeDistance #
numMyel = 100
numUnmyel = 350
poles = 2
poleDistance = 3 # mm
...
# tHomo = saveDict['t']
ts = [np.arange(0,40,0.0025), np.arange(0,20,0.0025)]
...
denoisedVoltage = w.wden(data[:,1], level=12, threshold=1.)
...
fieldStrings = ['homogeneous', 'FEM', 'idealized Cuff']
for fieldTypeInd in [0,1,2]: # fieldTypes:
    ...
    for typeInd in [typeIndSelected]:
        ...
        if fieldTypeInd == 0:
            ...
            raise ValueError('Field index not valid.')
        ...
        for fiberInd in range(numFibers):
            ...
            # plt.show()
            if typeInd == 1:
                CV = diameters[fiberInd] * 5 # CVs[fiberInd]
            else:
