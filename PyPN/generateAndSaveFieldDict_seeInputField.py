...
# sort folders and positions in ascending order
orderedIndices = np.argsort(axonXs)
axonXs = np.array(axonXs)[orderedIndices]
xPFolders = np.array(xPFolders)[orderedIndices]
...
def load_field(folder, axonXs):
    ...
    for xPInd, xPFolderName in enumerate(xPFolders):
        ...
        axonZSteps = len(axonZs)

        # sort file names and steps
        orderedIndices = np.argsort(axonZs)
        filenamesZP = np.array(filenamesZP)[orderedIndices]
        axonZs  = np.array(axonZs)[orderedIndices]
    ...
    fieldImage = np.zeros([xSteps, ySteps, zSteps, axonXSteps, 1, axonZSteps])
            ...
            for xInd in range(xSteps):
                for yInd in range(ySteps):
                    for zInd in range(zSteps):
                        ...
                        fieldImage[xInd, yInd, zInd, axonXInd, 0, axonZInd] = voltages[axonXInd][axonZInd][vIndexCalc]

    # correct units from um to m for axon positions
    axonXs = axonXs.astype(float) / 1000000
    axonZs = axonZs.astype(float) / 1000000
...
np.save(os.path.join(destinationFolder, 'fieldDictOrdered.npy'), fieldDict)
