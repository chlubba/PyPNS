...
tCutPlot = tCut[np.logical_and(tCut>timePlotMinRec, tCut<timePlotMaxRec)]
vDenCutPlot = vDenCut[np.logical_and(tCut>timePlotMinRec, tCut<timePlotMaxRec)]
...
for fieldTypeInd in [0]: # fieldTypes:
        ...
        for fiberInd in range(numFibers):
            ...
            for ii in range(int(max(wantedNums[fiberInd], 1))):
                ...
                for poleInd in range(poles):
                    ...
                    CAP = np.add(CAP, polePolarities[poleInd] * paddedSignal * 1**typeInd) # todo: scaling, be aware
    ...
    plt.plot(f_common, 20 * np.log10(
        np.sqrt(Pxx_den_rec_interp / np.max(Pxx_den_rec_interp))), label='rec', color='r')
    plt.plot(f_common, 20 * np.log10(
        np.sqrt(Pxx_den_cut / np.max(Pxx_den_cut))), label='sim', color='b')
