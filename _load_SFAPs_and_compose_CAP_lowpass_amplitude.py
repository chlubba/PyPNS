...
vDenCut = denoisedVoltage[time>tStart]/1000 # micro to milli Volt

# plt.plot(tCut, vDenCut/np.max(vDenCut), color='red', label='Experimental data')
# plt.show()

plt.plot(tCut[np.logical_and(tCut>timePlotMin, tCut<timePlotMax)], vDenCut[np.logical_and(tCut>timePlotMin, tCut<timePlotMax)], color='black', linewidth=2, label='Experimental data')
# plt.show()
...
for fieldTypeInd in [1]: # fieldTypes:
    ...
    plt.plot(tCAP[np.logical_and(tCAP > timePlotMin, tCAP < timePlotMax)],
             CAP[np.logical_and(tCAP > timePlotMin, tCAP < timePlotMax)]*100, ':', label='no filter')
    for order in [2,3]:
        filteredCAP = butter_lowpass_filter(CAP, cutoff, 1000./dt, order=order)
        plt.plot(tCAP[np.logical_and(tCAP>timePlotMin, tCAP<timePlotMax)], filteredCAP[np.logical_and(tCAP>timePlotMin, tCAP<timePlotMax)]*1000, label='order='+str(order))
