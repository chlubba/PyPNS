...
def loadVoltage_fromfile(filename):
    ...
    print np.shape(np.array(Vbrute))[0]
    numberOfSimulatedAxons = int(floor(np.shape(np.array(Vbrute))[0]/number_of_segments))

    V = np.zeros((numberOfSimulatedAxons,number_of_segments,len(t)))
    # V[0] = temp[1:len(temp)]
    for m in range(0, numberOfSimulatedAxons):

        # print 'axon '+str(m)
        # print 'shape of voltage array '+str(np.shape(Vbrute[1+m*number_of_segments][1:len(t)]))

        for k in range(1,number_of_segments):
            V[m][k]= Vbrute[k+m*number_of_segments][1:len(t)+1]
    return [t,V, numberOfSimulatedAxons]
...
for k in range(3):
    ...
    filename = tp[0][1]
    print filename
    [t,V,n] = loadVoltage_fromfile(filename)
    for m in [k]:#range(0,n):
        print 'm='+str(m)
        for j in range(number_of_segments):
            colorVal = scalarMap.to_rgba(j)
            # plt.figure()
            # plt.plot(np.array(V[m][j]))
            # plt.show()
            ax[k].plot(np.array(t), np.array(V[m][j]), color=colorVal)
            ax[k].set_xlim([5,30])
