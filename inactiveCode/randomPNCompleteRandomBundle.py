...
def create_random_axon(bundleCoords, bundleRadius, segmentLengthAxon, maximumAngle = 0.314):
    ...
    while currentBundleSegment <= bundleLength-1:
        ...
        factorAxonDirection = 1 - 1/(1+np.exp(-20*(distance/bundleRadius - 0.8)))
...
bundleOrientationCoords = np.empty([bundleLength, 3])
bundleOrientationCoords[:,0] = range(bundleLength)
bundleOrientationCoords[:,1] = np.concatenate((np.zeros(bundleLength/2),range(bundleLength/2)))
bundleOrientationCoords[:,2] = np.concatenate((np.zeros(bundleLength/2),range(bundleLength/2)))

bundleCoords = create_random_axon(bundleOrientationCoords, 10, 5)
...
for axonIndex in range(numberOfAxons):
    ...
    coords = create_random_axon(bundleCoords, bundleRadius, segmentLengthAxon, 0.5)
...
ax.plot(bundleCoords[:,0], bundleCoords[:,1], bundleCoords[:,2], label='Bundle center', linewidth=3.0)
ax.legend()
