    ...
    for i in [0,1]:
        ...
        for profileInd in [0,1,2]: # [0, 1, 2]
            ...
            for widthInd, cuffWidth in enumerate(cuffWidths):
                ...
                zValues = np.linspace(-0.015, 0.015, 10000)

                if profileInd == 0:
                    profileTitle = 'idealized cuff field. Length in mm'
                    profileLegends = (np.array(cuffWidths) * 2000).astype(str)

                    vValues = np.maximum(0, np.multiply(np.subtract(1, np.abs(zValues / cuffWidth)), 8.83e-5))

                elif profileInd == 1:

                    profileTitle = 'gaussian, sigma in mm'
                    profileLegends = (np.array(cuffWidths) * 500).astype(str)

                    mu = 0.
                    vValues = 1/(cuffWidth/2 * np.sqrt(2 * np.pi)) * np.exp( - (zValues - mu)**2 / (2 * (cuffWidth/2)**2) )
                ...
                axarr[profileInd][0].plot(zValues, vValues, label=profileLegends[widthInd], color=colorVal)
                axarr[profileInd][0].set_title('phi over z')
                axarr[profileInd][0].set_xlabel('z (m)')
                axarr[profileInd][0].set_ylabel('V (mV)')
            ...
            for recMechIndex in range(len(bundle.recordingMechanisms)):
                ...
                axarr[profileInd][i+1].plot(t, SFAPs, label=str(cuffWidths[recMechIndex]*2*1000)+' mm', color=colorVal)

            axarr[profileInd][i+1].set_xlabel('time [ms]')
            axarr[profileInd][i+1].set_ylabel('$V_{ext}$ [mV]')
            ...
            if i == 0:
                axarr[profileInd][i+1].set_xlim([0,70]) # [24,32]
