...
for unmyelinatedDiam in unmyelinatedDiams:
    ...
    ax2.semilogy(distances, vpps, label=str(unmyelinatedDiam)+'um', color=diamColors[numDiams-1-diamIndex,:])
    ax2.plot([minDistance, maxDistance], np.tile(vppsMono[diamIndex], [2,1]),  '--', color=diamColors[numDiams-1-diamIndex,:])

    # plt.show()

    diamIndex += 1

ax2.set_title('Vpp over distance of electrode poles (unmyelinated)')
ax2.set_xlabel('electrode distance [um]')
