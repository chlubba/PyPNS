...
diametersMyel = np.concatenate((np.arange(1., 3., 0.1), np.arange(3., 5., 0.25)))  # np.arange(0.4, 1.1, 0.02)
...
if calculationFlag:
    ...
    for i in [0,1]:
        ...
        for diameterInd, diameter in enumerate(diameters):
            ...
            LFPMech.append(PyPN.Extracellular.precomputedFEM_symmetrical_inhomogeneity(bundle.bundleCoords))
            ...
            if i == 1:
                ...
                recordingParametersNew = {'bundleGuide': bundle.bundleCoords,
                                          ...
                                          'positionAlongBundle': np.floor(bundleLengths[i]*0.8 / bundle.axons[0].lengthOneCycle) *
                                                                 bundle.axons[0].lengthOneCycle + bundle.axons[0].lengthOneCycle*0.5,
                                          'numberOfPoles': 1,
                                          'poleDistance': 1000,
                                          }
...
pickle.dump(saveDict, open(os.path.join('/media/carl/4ECC-1C44/PyPN/SFAPs', 'SFAPsOilMicroscopyDiams.dict'), "wb"))
